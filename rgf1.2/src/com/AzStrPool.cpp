...
void AzStrPool::reduce(int min_count)
{
  ...
  for (ex = 0; ex < ent_num; ++ex) {
    if (ent[ex].count >= min_count) {
      ...
      if (ent[ex].offs != new_offs) {
        ent[new_ex].offs = new_offs; 
        AzBytArr str_temp = AzBytArr(data + ent[ex].offs, ent[ex].len); 
        memcpy(data + new_offs, str_temp.point(), str_temp.getLen()); 
        new_offs += str_temp.getLen(); 
        *(data + new_offs) = '\0'; 
        ++new_offs; 
      }
