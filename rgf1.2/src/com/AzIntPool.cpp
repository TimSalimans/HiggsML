...
int rkj_compare_IP_ent(const void *v1, const void *v2); 
...
void AzIntPool::_swap()
{
  ...
  for (ex = 0; ex < data_num; ++ex) {
    AzFile::swap_long(&data[ex]); 
  }

  for (ex = 0; ex < ent_num; ++ex) {
    AzIpEnt *ep = &ent[ex]; 
    AzFile::swap_long(&ep->offs); 
    AzFile::swap_long(&ep->num); 
...
/*------------------------------------------------------------------*/
int AzIntPool::getCount(int ent_no) const
{
...
/*------------------------------------------------------------------*/
void AzIntPool::setCount(int ent_no, int new_count) 
{
...
void AzIntPool::commit()  
{
  ...
  qsort(ent, ent_num, sizeof(ent[0]), rkj_compare_IP_ent); 
  ...
  for (ex = 0; ex < ent_num; ) {
    ...
    int count = 0; 
    for ( ; ex < ent_num; ++ex) {
      if (rkj_compare_IP_ent(&ent[ex], &ent[new_id]) != 0) {
        break;
      }
                         ...
                         int ints_num) const
{
  ...
  for ( ; ; ) {
    if (lx > hx) break;
    int mx = (lx + hx) / 2; 
    int cmp = rkj_compare_IP_ent(&inp, &ent[mx]); 
...
/*------------------------------------------------------------------*/
int AzIntPool::inc_data(int min_inc)
{
  int inc = MIN(1024*1024*2, MAX(2048, data_num)); 
