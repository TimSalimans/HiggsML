...
/*--------------------------------------------------------*/
int AzTree::write(AzFile *file)
{
  int len = 0; 
  len += file->writeInt(root_nx); 
  len += file->writeInt(nodes_used); 
  int nx; 
  for (nx = 0; nx < nodes_used; ++nx) {
    len += nodes[nx].write(file); 
  }
  return len; 
}
...
/*--------------------------------------------------------*/
int AzTreeNode::write(AzFile *file) 
{
  int len = 0; 
  len += file->writeDouble(border_val); 
  len += file->writeDouble(weight); 
  len += file->writeInt(fx); 
  len += file->writeInt(le_nx); 
  len += file->writeInt(gt_nx); 
  len += file->writeInt(parent_nx); 
  return len; 
}
